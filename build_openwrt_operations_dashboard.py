...
def build_dashboard() -> dict[str, Any]:
    ...
    b.add(overview, stat(6, "Telemetry Freshness", f'max(time() - node_textfile_mtime_seconds{{{PROM_FILTER}}}) or vector(999999)', "s", "Age of the custom textfile metrics. Normal cadence is roughly one minute.", thresholds_key="freshness", graph=True), 20, 0, 4, 4)
    ...
    b.add(health, stat(412, "Textfile Age", f'max(time() - node_textfile_mtime_seconds{{{PROM_FILTER}}}) or vector(999999)', "s", "Age of custom textfile metrics.", thresholds_key="freshness", graph=True), 18, 26, 6, 5)
